...
fn undo_list_is_read_only() {
    ...
    temper(home.path(), fh.path(), st.path())
        .arg("install")
        .assert()
        .success();
...
fn undo_targets_named_run() {
    ...
    let out = temper(home.path(), fh.path(), st.path())
        .args(["undo", "--list", "--json"])
        .output()
        .unwrap();
...
fn setkey_json_refuses_scalar_intermediate() {
    ...
    assert!(
        fs::read_to_string(&target).unwrap().contains("\"a\": 5"),
        "scalar was clobbered"
    );
}
